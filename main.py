import tkinter as tk
import customtkinter
import os
import pyaudio
import wave
import time
import threading
from functions import *
from recorder import *

customtkinter.set_appearance_mode("System")  # Modes: "System" (standard), "Dark", "Light"
customtkinter.set_default_color_theme("green")  # Themes: "blue" (standard), "green", "dark-blue"

class App(customtkinter.CTk):
    path = None

    def __init__(self):
        super().__init__()


        # configure window
        self.title("Voice Warping Application")
        self.geometry(f"{1830}x{680}")

        self.grid_columnconfigure(1, weight=1)
        self.grid_columnconfigure((2, 3), weight=0)
        self.grid_rowconfigure((1, 2), weight=1)


        # create Save_button
        self.main_button_1 = customtkinter.CTkButton(self, border_width=2, text_color=("gray10", "#DCE4EE"),text="Save as",width=170,height=50, command=saveTo)
        self.main_button_1.grid(row=0, column=3, padx=(0, 370), pady=(0, 270))

        #Create Spectrogram Button
        self.Show_Spectrogram = customtkinter.CTkButton(self, border_width=2, text_color=("gray10", "#DCE4EE"),text="Show Graphs",width=170,height=50, command=showGraphs)
        self.Show_Spectrogram.grid(row=0, column=3, padx=(0, 370), pady=(0, 100))
        

        #General Frame

        self.General_frame = customtkinter.CTkFrame(self)
        self.General_frame.grid(row=0, column=0, padx=(40, 20), pady=(20, 200))


        # Warping Part
        self.optionmenu_1 = customtkinter.CTkOptionMenu(self.General_frame, dynamic_resizing=False,width=250,height=50,values=["Default", "Angry", "Sad", "Happy", "Fear", "Disgust", "Robot", "Kid", "Ogre", "Telephone", "Cave"], command=self.changePreset)
        self.optionmenu_1.grid(row=0, column=0, padx=20, pady=(20, 10))
        self.Upload_Button = customtkinter.CTkButton(self.General_frame, border_width=2, text_color=("gray10", "#DCE4EE"),text="Upload",width=200, height=50, command=getAudio)
        self.Upload_Button.grid(row=0, column=2, padx=(20, 20), pady=(20, 20),)
        self.Change_Button = customtkinter.CTkButton(self.General_frame, border_width=2,text_color=("gray10", "#DCE4EE"), text="Change", width=200, height=50, command=lambda: [changeAudio(isAllpass.get(), isComb.get(), isEcho.get(), isReverbed.get(), isChorus.get(), isFlanger.get(), isVibrato.get(), isTremolo.get(), isFade.get(), isFadeIn.get(), isFadeOut.get(), isTpl.get(), isEQ.get(), currentWNoise.get(), currentStrecth.get(), currentPitch.get(), currentIntensity.get(), currentHarmonic.get(), currentPercussive.get(), hz_32.get(), hz_63.get(), hz_125.get(), hz_250.get(), hz_500.get(), hz_1000.get(), hz_2000.get(), hz_4000.get(), hz_8000.get(), hz_16000.get())])
        self.Change_Button.grid(row=3, column=1, padx=(700, 0), pady=(20, 20))
        self.Play_Button = customtkinter.CTkButton(self.General_frame, border_width=2,text_color=("gray10", "#DCE4EE"), text="Play",width=200, height=50, command=play)
        self.Play_Button.grid(row=3, column=2, padx=(0, 0), pady=(20, 20))

        # Record Button
        self.recrodbutton_frame = customtkinter.CTkFrame(self)
        self.recrodbutton_frame.grid(row=0, column=3, padx=(0, 380), pady=(67,618))
        self.textbox = customtkinter.CTkLabel(self.recrodbutton_frame, text="Record",text_color="#fff",width=250,justify="left",anchor="w",font=("Arial", 25))
        self.textbox.grid(row=0, column=0, padx=(20,0),pady=(20,0),sticky="nsew")
        self.record_button_1 = customtkinter.CTkButton(master=self.recrodbutton_frame, border_width=2, text_color=("gray10", "#DCE4EE"), text="Save to", command=self.choosePath)
        self.record_button_1.grid(row=1, column=0, padx=(20, 100), pady=(20, 20), )
        self.record_button_2 = customtkinter.CTkButton(master=self.recrodbutton_frame, border_width=2, fg_color="#2fa572", text_color=("gray10", "#DCE4EE"), text="Record (Start/Stop)", command=self.click_handler)
        self.recording = False
        self.record_button_2.grid(row=2, column=0, padx=(20, 100), pady=(20, 20), )

        # create slider and progressbar frame
        currentWNoise = tk.DoubleVar()
        strcurrentWNoise = tk.StringVar()
        strcurrentWNoise.set('0')
        currentStrecth = tk.DoubleVar()
        strcurrentStrecth = tk.StringVar()
        strcurrentStrecth.set('1')
        currentPitch = tk.DoubleVar()
        strcurrentPitch = tk.StringVar()
        strcurrentPitch.set('0')
        currentIntensity = tk.DoubleVar()
        currentHarmonic = tk.DoubleVar()
        strcurrentHarmonic = tk.StringVar()
        strcurrentHarmonic.set('1')
        currentPercussive = tk.DoubleVar()
        strcurrentPercussive = tk.StringVar()
        strcurrentPercussive.set('1')
        self.Slider_frame_horizontal = customtkinter.CTkFrame(self.General_frame)
        self.Slider_frame_horizontal.grid(row=1, column=0,padx=(25,0), sticky="nsew")
         # white noise 
        self.text = customtkinter.CTkLabel(self.Slider_frame_horizontal, text="White Noise", text_color="#fff", width=250, anchor="w", font=("Arial", 17))
        self.text.grid(row=0, column=0,padx=(10,0),pady=(10,0), sticky="nsew")

        self.slider = customtkinter.CTkSlider(self.Slider_frame_horizontal, orientation='horizontal', from_=0, to=0.05, variable=currentWNoise, command=lambda event:[self.text_whitenoise.configure(textvariable=strcurrentWNoise), strcurrentWNoise.set(f"{currentWNoise.get():.3f}")])
        self.slider.grid(row=1, column=0,  sticky="ew")
        self.text_whitenoise = customtkinter.CTkLabel(self.Slider_frame_horizontal, textvariable=strcurrentWNoise)
        self.text_whitenoise.grid(row=2, column=0,padx=(0,0),pady=(0,0))
        
        #Stretch
        self.text = customtkinter.CTkLabel(self.Slider_frame_horizontal, text="Stretch", text_color="#fff", width=250, anchor="w", font=("Arial", 17))
        self.text.grid(row=3, column=0,padx=(0,0), sticky="nsew")

        
        self.slider_2 = customtkinter.CTkSlider(self.Slider_frame_horizontal, orientation='horizontal', from_=0.5, to=2, variable=currentStrecth, command=lambda event:[self.text_stretch.configure(textvariable=strcurrentStrecth), strcurrentStrecth.set(f"{currentStrecth.get():.3f}")])
        self.slider_2.set(1)
        self.slider_2.grid(row=4, column=0,  sticky="ew")

        self.text_stretch = customtkinter.CTkLabel(self.Slider_frame_horizontal, textvariable=strcurrentStrecth)
        self.text_stretch.grid(row=5, column=0,padx=(0,0),pady=(0,0))

        #Pitch
        self.text = customtkinter.CTkLabel(self.Slider_frame_horizontal, text="Pitch", text_color="#fff", width=250,justify="left", anchor="w", font=("Arial", 17))
        self.text.grid(row=6, column=0,padx=(10,0), sticky="nsew")
        self.slider_3 = customtkinter.CTkSlider(self.Slider_frame_horizontal, orientation='horizontal', from_=-10, to=10, variable=currentPitch, command=lambda event:[self.text_pitch.configure(textvariable=strcurrentPitch), strcurrentPitch.set(f"{currentPitch.get():.3f}")])
        self.slider_3.set(0)
        self.slider_3.grid(row=7, column=0, sticky="ew")

        self.text_pitch = customtkinter.CTkLabel(self.Slider_frame_horizontal, textvariable=strcurrentPitch)
        self.text_pitch.grid(row=8, column=0,padx=(0,0),pady=(0,0))

        #Harmonic
        self.text = customtkinter.CTkLabel(self.Slider_frame_horizontal, text="Harmonic", text_color="#fff", width=250,justify="center", anchor="w", font=("Arial", 17))
        self.text.grid(row=10, column=0,padx=(10,0), sticky="nsew")
        self.slider_4 = customtkinter.CTkSlider(self.Slider_frame_horizontal, orientation='horizontal', from_=-10, to=10, variable=currentHarmonic,command=lambda event:[self.text_harmonic.configure(textvariable=strcurrentHarmonic),strcurrentHarmonic.set(f"{currentHarmonic.get():.3f}")])
        self.slider_4.set(1)
        self.slider_4.grid(row=11, column=0, sticky="ew")
        self.text_harmonic = customtkinter.CTkLabel(self.Slider_frame_horizontal, textvariable=strcurrentHarmonic)
        self.text_harmonic.grid(row=12, column=0,padx=(0,0),pady=(0,0))

        #Percussive
        self.text = customtkinter.CTkLabel(self.Slider_frame_horizontal, text="Percussive", text_color="#fff", width=250,justify="center", anchor="w", font=("Arial", 17))
        self.text.grid(row=13,column=0,padx=(10,0), sticky="nsew")
        self.slider_5 = customtkinter.CTkSlider(self.Slider_frame_horizontal, orientation='horizontal', from_=-10, to=10, variable=currentPercussive,command=lambda event:[self.text_percussive.configure(textvariable=strcurrentPercussive),strcurrentPercussive.set(f"{currentPercussive.get():.3f}")])
        self.slider_5.set(1)
        self.slider_5.grid(row=14, column=0, sticky="ew")
        self.text_percussive = customtkinter.CTkLabel(self.Slider_frame_horizontal, textvariable=strcurrentPercussive)
        self.text_percussive.grid(row=15, column=0,padx=(0,0),pady=(0,0))

        # Equlizer eqSliders
        isEQ, hz_32, hz_63, hz_125, hz_250, hz_500, hz_1000, hz_2000, hz_4000, hz_8000, hz_16000 = tk.IntVar(), tk.DoubleVar(), tk.DoubleVar(), tk.DoubleVar(), tk.DoubleVar(), tk.DoubleVar(), tk.DoubleVar(), tk.DoubleVar(), tk.DoubleVar(), tk.DoubleVar(), tk.DoubleVar()
        strhz_32, strhz_63, strhz_125, strhz_250, strhz_500, strhz_1000, strhz_2000, strhz_4000, strhz_8000, strhz_16000 = tk.StringVar(), tk.StringVar(), tk.StringVar(), tk.StringVar(), tk.StringVar(), tk.StringVar(), tk.StringVar(), tk.StringVar(), tk.StringVar(), tk.StringVar()
        strhz_32.set('0')
        strhz_63.set('0')
        strhz_125.set('0')
        strhz_250.set('0')
        strhz_500.set('0')
        strhz_1000.set('0')
        strhz_2000.set('0')
        strhz_4000.set('0')
        strhz_8000.set('0')
        strhz_16000.set('0')
        self.Slider_frame_Equalizer = customtkinter.CTkFrame(self.General_frame)
        self.Slider_frame_Equalizer.grid(row=1, column=1, padx=(50,50))

        self.checkbox_Equalizer = customtkinter.CTkCheckBox(master=self.Slider_frame_Equalizer, text="Equalizer", font=("Arial", 25), variable=isEQ)
        self.checkbox_Equalizer.grid(row=0, column=0, pady=(20, 40), padx=(20,0), sticky="n")
        #32hz
        self.text_hz_32 = customtkinter.CTkLabel(self.Slider_frame_Equalizer, textvariable=strhz_32)
        self.text_hz_32.grid(row=1, column=0,padx=(30,0),pady=(0,230))
        self.eqslider_1 = customtkinter.CTkSlider(self.Slider_frame_Equalizer, orientation="vertical", from_=-20, to=20, variable=hz_32, command=lambda event:[self.text_hz_32.configure(textvariable=strhz_32) ,strhz_32.set(f"{hz_32.get():.1f}")])
        self.eqslider_1.grid(row=1, column=0, padx=(30, 0), pady=(10, 10))
        self.text = customtkinter.CTkLabel(self.Slider_frame_Equalizer, text="32hz")
        self.text.grid(row=2, column=0,padx=(30,0),pady=(0,0))
        #63hz
        self.text_hz_63 = customtkinter.CTkLabel(self.Slider_frame_Equalizer, textvariable=strhz_63)
        self.text_hz_63.grid(row=1, column=1,padx=(0,40),pady=(0,230))
        self.eqslider_2 = customtkinter.CTkSlider(self.Slider_frame_Equalizer, orientation="vertical", from_=-20, to=20, variable=hz_63, command=lambda event:[self.text_hz_63.configure(textvariable=strhz_63) ,strhz_63.set(f"{hz_63.get():.1f}")])
        self.eqslider_2.grid(row=1, column=1, padx=(0, 40), pady=(10, 10))
        self.text = customtkinter.CTkLabel(self.Slider_frame_Equalizer, text="63hz")
        self.text.grid(row=2, column=1,padx=(0,40),pady=(0,0))
        #125hz
        self.text_hz_125 = customtkinter.CTkLabel(self.Slider_frame_Equalizer, textvariable=strhz_125)
        self.text_hz_125.grid(row=1, column=2,padx=(0,40),pady=(0,230))
        self.eqslider_3 = customtkinter.CTkSlider(self.Slider_frame_Equalizer, orientation="vertical", from_=-20, to=20, variable=hz_125, command=lambda event:[self.text_hz_125.configure(textvariable=strhz_125), strhz_125.set(f"{hz_125.get():.1f}")])
        self.eqslider_3.grid(row=1, column=2, padx=(0, 40), pady=(10, 10))
        self.text = customtkinter.CTkLabel(self.Slider_frame_Equalizer, text="125hz")
        self.text.grid(row=2, column=2,padx=(0,40),pady=(0,0))
        #250hz
        self.text_hz_250 = customtkinter.CTkLabel(self.Slider_frame_Equalizer, textvariable=strhz_250)
        self.text_hz_250.grid(row=1, column=3,padx=(0,40),pady=(0,230))
        self.eqslider_4 = customtkinter.CTkSlider(self.Slider_frame_Equalizer, orientation="vertical", from_=-20, to=20, variable=hz_250, command=lambda event:[self.text_hz_250.configure(textvariable=strhz_250),strhz_250.set(f"{hz_250.get():.1f}")])
        self.eqslider_4.grid(row=1, column=3, padx=(0, 40), pady=(10, 10))
        self.text = customtkinter.CTkLabel(self.Slider_frame_Equalizer, text="250hz")
        self.text.grid(row=2, column=3,padx=(0,40),pady=(0,0))
        #500hz
        self.text_hz_500 = customtkinter.CTkLabel(self.Slider_frame_Equalizer, textvariable=strhz_500)
        self.text_hz_500.grid(row=1, column=4,padx=(0,40),pady=(0,230))
        self.eqslider_5 = customtkinter.CTkSlider(self.Slider_frame_Equalizer, orientation="vertical", from_=-20, to=20, variable=hz_500, command=lambda event:[self.text_hz_500.configure(textvariable=strhz_500), strhz_500.set(f"{hz_500.get():.1f}")])
        self.eqslider_5.grid(row=1, column=4, padx=(0, 40), pady=(10, 10))
        self.text = customtkinter.CTkLabel(self.Slider_frame_Equalizer, text="500hz")
        self.text.grid(row=2, column=4,padx=(0,40),pady=(0,0))
        #1000hz
        self.text_hz_1000 = customtkinter.CTkLabel(self.Slider_frame_Equalizer, textvariable=strhz_1000)
        self.text_hz_1000.grid(row=1, column=5,padx=(0,40),pady=(0,230))
        self.eqslider_6 = customtkinter.CTkSlider(self.Slider_frame_Equalizer, orientation="vertical", from_=-20, to=20, variable=hz_1000, command= lambda event:[self.text_hz_1000.configure(textvariable=strhz_1000), strhz_1000.set(f"{hz_1000.get():.1f}")])
        self.eqslider_6.grid(row=1, column=5, padx=(0, 40), pady=(10, 10))
        self.text = customtkinter.CTkLabel(self.Slider_frame_Equalizer, text="1khz")
        self.text.grid(row=2, column=5,padx=(0,40),pady=(0,0))
        #2000hz
        self.text_hz_2000 = customtkinter.CTkLabel(self.Slider_frame_Equalizer, textvariable=strhz_2000)
        self.text_hz_2000.grid(row=1, column=6,padx=(0,40),pady=(0,230))
        self.eqslider_7 = customtkinter.CTkSlider(self.Slider_frame_Equalizer, orientation="vertical", from_=-20, to=20, variable=hz_2000, command=lambda event:[self.text_hz_2000.configure(textvariable=strhz_2000), strhz_2000.set(f"{hz_2000.get():.1f}")])
        self.eqslider_7.grid(row=1, column=6, padx=(0, 40), pady=(10, 10))
        self.text = customtkinter.CTkLabel(self.Slider_frame_Equalizer, text="2khz")
        self.text.grid(row=2, column=6,padx=(0,40),pady=(0,0))
        #4000hz
        self.text_hz_4000 = customtkinter.CTkLabel(self.Slider_frame_Equalizer, textvariable=strhz_4000)
        self.text_hz_4000.grid(row=1, column=7,padx=(0,40),pady=(0,230))
        self.eqslider_8 = customtkinter.CTkSlider(self.Slider_frame_Equalizer, orientation="vertical", from_=-20, to=20, variable=hz_4000, command=lambda event:[self.text_hz_4000.configure(textvariable=strhz_4000), strhz_4000.set(f"{hz_4000.get():.1f}")])
        self.eqslider_8.grid(row=1, column=7, padx=(0, 40), pady=(10, 10))

        self.text = customtkinter.CTkLabel(self.Slider_frame_Equalizer, text="4khz")
        self.text.grid(row=2, column=7,padx=(0,40),pady=(0,0))
        #8000hz
        self.text_hz_8000 = customtkinter.CTkLabel(self.Slider_frame_Equalizer, textvariable=strhz_8000)
        self.text_hz_8000.grid(row=1, column=8,padx=(0,40),pady=(0,230))
        self.eqslider_9 = customtkinter.CTkSlider(self.Slider_frame_Equalizer, orientation="vertical", from_=-20, to=20, variable=hz_8000, command= lambda event:[self.text_hz_8000.configure(textvariable=strhz_8000), strhz_8000.set(f"{hz_8000.get():.1f}")])
        self.eqslider_9.grid(row=1, column=8, padx=(0, 40), pady=(10, 10))

        self.text = customtkinter.CTkLabel(self.Slider_frame_Equalizer, text="8khz")
        self.text.grid(row=2, column=8,padx=(0,40),pady=(0,0))
        #16000hz
        self.text_hz_16000 = customtkinter.CTkLabel(self.Slider_frame_Equalizer, textvariable=strhz_16000)
        self.text_hz_16000.grid(row=1, column=9,padx=(0,40),pady=(0,230))
        self.eqslider_10 = customtkinter.CTkSlider(self.Slider_frame_Equalizer, orientation="vertical", from_=-20, to=20, variable=hz_16000,command=lambda event:[self.text_hz_16000.configure(textvariable=strhz_16000), strhz_16000.set(f"{hz_16000.get():.1f}")])
        self.eqslider_10.grid(row=1, column=9, padx=(0, 40), pady=(10, 10))

        self.text = customtkinter.CTkLabel(self.Slider_frame_Equalizer, text="16khz")
        self.text.grid(row=2, column=9,padx=(0,40),pady=(0,0))


        # create checkbox and switch frame
        isAllpass = tk.IntVar()
        isComb = tk.IntVar()
        isEcho = tk.IntVar()
        isReverbed = tk.IntVar()
        isChorus = tk.IntVar()
        isFlanger = tk.IntVar()
        isVibrato = tk.IntVar()
        isTremolo = tk.IntVar()
        isFade = tk.IntVar()
        isFadeIn = tk.IntVar()
        isFadeOut = tk.IntVar()
        isTpl = tk.IntVar()
        self.checkbox_slider_frame = customtkinter.CTkFrame(self.General_frame)
        self.checkbox_slider_frame.grid(row=1, column=2, padx=(0, 20))
        self.checkbox_1 = customtkinter.CTkCheckBox(master=self.checkbox_slider_frame,text="Allpass", variable=isAllpass)
        self.checkbox_1.grid(row=1, column=1, pady=(34, 15), padx=20, sticky="n")
        self.checkbox_2 = customtkinter.CTkCheckBox(master=self.checkbox_slider_frame,text="Comb", variable=isComb)
        self.checkbox_2.grid(row=1, column=0, pady=(34, 15), padx=20, sticky="n")
        self.checkbox_3 = customtkinter.CTkCheckBox(master=self.checkbox_slider_frame,text="Echo", variable=isEcho)
        self.checkbox_3.grid(row=2, column=0, pady=(0,15), padx=20, sticky="n")
        self.checkbox_4 = customtkinter.CTkCheckBox(master=self.checkbox_slider_frame,text="Reverbed", variable=isReverbed)
        self.checkbox_4.grid(row=2, column=1, pady=(0, 15), padx=20, sticky="n")
        self.checkbox_5 = customtkinter.CTkCheckBox(master=self.checkbox_slider_frame,text="Chorus", variable=isChorus)
        self.checkbox_5.grid(row=3, column=0, pady=(0, 15), padx=20, sticky="n")
        self.checkbox_6 = customtkinter.CTkCheckBox(master=self.checkbox_slider_frame,text="Flanger", variable=isFlanger)
        self.checkbox_6.grid(row=3, column=1, pady=(0, 15), padx=20, sticky="n")
        self.checkbox_7 = customtkinter.CTkCheckBox(master=self.checkbox_slider_frame,text="Vibrato", variable=isVibrato)
        self.checkbox_7.grid(row=4, column=0, pady=(0,15), padx=20, sticky="n")
        self.checkbox_8 = customtkinter.CTkCheckBox(master=self.checkbox_slider_frame,text="Tremolo", variable=isTremolo)
        self.checkbox_8.grid(row=4, column=1, pady=(0, 15), padx=20, sticky="n")
        self.checkbox_9 = customtkinter.CTkCheckBox(master=self.checkbox_slider_frame,text="Fade", variable=isFade)
        self.checkbox_9.grid(row=5, column=0, pady=(0, 15), padx=20, sticky="n")
        self.checkbox_10 = customtkinter.CTkCheckBox(master=self.checkbox_slider_frame,text="Fade In", variable=isFadeIn)
        self.checkbox_10.grid(row=5, column=1, pady=(0, 15), padx=20, sticky="n")
        self.checkbox_11 = customtkinter.CTkCheckBox(master=self.checkbox_slider_frame,text="Fade Out", variable=isFadeOut)
        self.checkbox_11.grid(row=6, column=0, pady=(0, 34), padx=20, sticky="n")
        self.checkbox_12 = customtkinter.CTkCheckBox(master=self.checkbox_slider_frame,text="TPL", variable=isTpl)
        self.checkbox_12.grid(row=6, column=1, pady=(0, 34), padx=20, sticky="n")
        # set default values
        self.optionmenu_1.set("Default")
        self.checkbox_1.select()
    
    def click_handler(self):
        if self.recording:
            self.recording = False
            self.record_button_2.configure(fg_color ="#2fa572")
        else:
            self.recording = True
            self.record_button_2.configure(fg_color ="#fe0000")
            threading.Thread(target=self.rec).start()
            
    def changePreset(self, preset):
        whitenoisestr = tk.StringVar()
        stretchstr = tk.StringVar()
        pitchstr = tk.StringVar()
        harmonicstr = tk.StringVar()
        percussivestr = tk.StringVar()
        hz32str = tk.StringVar()
        hz63str = tk.StringVar()
        hz125str = tk.StringVar()
        hz250str = tk.StringVar()
        hz500str = tk.StringVar()
        hz1000str = tk.StringVar()
        hz2000str = tk.StringVar()
        hz4000str = tk.StringVar()
        hz8000str = tk.StringVar()
        hz16000str = tk.StringVar()
        if preset == "Angry":
            self.slider.set(0)
            whitenoisestr.set('0')
            self.text_whitenoise.configure(textvariable=whitenoisestr)
            self.slider_2.set(1.4)
            stretchstr.set('1.4')
            self.text_stretch.configure(textvariable=stretchstr)
            self.slider_3.set(-0.5)
            pitchstr.set('-0.5')
            self.text_pitch.configure(textvariable=pitchstr)
            self.slider_4.set(1)
            harmonicstr.set('1')
            self.text_harmonic.configure(textvariable=harmonicstr)
            self.slider_5.set(3)
            percussivestr.set('3')
            self.text_percussive.configure(textvariable=percussivestr)
            self.checkbox_Equalizer.select()
            self.eqslider_1.set(-10)
            hz32str.set('-10')
            self.text_hz_32.configure(textvariable=hz32str)
            self.eqslider_2.set(-10)
            hz63str.set('-10')
            self.text_hz_63.configure(textvariable=hz63str)
            self.eqslider_3.set(15)
            hz125str.set('15')
            self.text_hz_125.configure(textvariable=hz125str)
            self.eqslider_4.set(10)
            hz250str.set('10')
            self.text_hz_250.configure(textvariable=hz250str)
            self.eqslider_5.set(7)
            hz500str.set('7')
            self.text_hz_500.configure(textvariable=hz500str)
            self.eqslider_6.set(5)
            hz1000str.set('5')
            self.text_hz_1000.configure(textvariable=hz1000str)
            self.eqslider_7.set(5)
            hz2000str.set('5')
            self.text_hz_2000.configure(textvariable=hz2000str)
            self.eqslider_8.set(10)
            hz4000str.set('10')
            self.text_hz_4000.configure(textvariable=hz4000str)
            self.eqslider_9.set(-8)
            hz8000str.set('-8')
            self.text_hz_8000.configure(textvariable=hz8000str)
            self.eqslider_10.set(-8)
            hz16000str.set('-8')
            self.text_hz_16000.configure(textvariable=hz16000str)
            self.checkbox_1.select()
            self.checkbox_2.deselect()
            self.checkbox_3.deselect()
            self.checkbox_4.deselect()
            self.checkbox_5.deselect()
            self.checkbox_6.deselect()
            self.checkbox_7.deselect()
            self.checkbox_8.deselect()
            self.checkbox_9.deselect()
            self.checkbox_10.deselect()
            self.checkbox_11.deselect()
            self.checkbox_12.deselect()
        if preset == "Sad":
            self.slider.set(0)
            whitenoisestr.set('0')
            self.text_whitenoise.configure(textvariable=whitenoisestr)
            self.slider_2.set(0.8)
            stretchstr.set('0.8')
            self.text_stretch.configure(textvariable=stretchstr)
            self.slider_3.set(1.2)
            pitchstr.set('1.2')
            self.text_pitch.configure(textvariable=pitchstr)
            self.slider_4.set(2)
            harmonicstr.set('2')
            self.text_harmonic.configure(textvariable=harmonicstr)
            self.slider_5.set(1)
            percussivestr.set('1')
            self.text_percussive.configure(textvariable=percussivestr)
            self.checkbox_Equalizer.select()
            self.eqslider_1.set(-20)
            hz32str.set('-20')
            self.text_hz_32.configure(textvariable=hz32str)
            self.eqslider_2.set(-10)
            hz63str.set('-10')
            self.text_hz_63.configure(textvariable=hz63str)
            self.eqslider_3.set(-10)
            hz125str.set('-10')
            self.text_hz_125.configure(textvariable=hz125str)
            self.eqslider_4.set(-5)
            hz250str.set('-5')
            self.text_hz_250.configure(textvariable=hz250str)
            self.eqslider_5.set(5)
            hz500str.set('5')
            self.text_hz_500.configure(textvariable=hz500str)
            self.eqslider_6.set(5)
            hz1000str.set('5')
            self.text_hz_1000.configure(textvariable=hz1000str)
            self.eqslider_7.set(10)
            hz2000str.set('10')
            self.text_hz_2000.configure(textvariable=hz2000str)
            self.eqslider_8.set(5)
            hz4000str.set('5')
            self.text_hz_4000.configure(textvariable=hz4000str)
            self.eqslider_9.set(6)
            hz8000str.set('6')
            self.text_hz_8000.configure(textvariable=hz8000str)
            self.eqslider_10.set(-8)
            hz16000str.set('-8')
            self.checkbox_1.select()
            self.checkbox_2.deselect()
            self.checkbox_3.deselect()
            self.checkbox_4.deselect()
            self.checkbox_5.deselect()
            self.checkbox_6.deselect()
            self.checkbox_7.deselect()
            self.checkbox_8.deselect()
            self.checkbox_9.deselect()
            self.checkbox_10.deselect()
            self.checkbox_11.deselect()
            self.checkbox_12.deselect()
        if preset == "Happy":
            self.slider.set(0)
            whitenoisestr.set('0')
            self.text_whitenoise.configure(textvariable=whitenoisestr)
            self.slider_2.set(1.6)
            stretchstr.set('1.6')
            self.text_stretch.configure(textvariable=stretchstr)
            self.slider_3.set(0.7)
            pitchstr.set('0.7')
            self.text_pitch.configure(textvariable=pitchstr)
            self.slider_4.set(1)
            harmonicstr.set('1')
            self.text_harmonic.configure(textvariable=harmonicstr)
            self.slider_5.set(4)
            percussivestr.set('4')
            self.text_percussive.configure(textvariable=percussivestr)
            self.checkbox_Equalizer.select()
            self.eqslider_1.set(-20)
            hz32str.set('-20')
            self.text_hz_32.configure(textvariable=hz32str)
            self.eqslider_2.set(-20)
            hz63str.set('-20')
            self.text_hz_63.configure(textvariable=hz63str)
            self.eqslider_3.set(-10)
            hz125str.set('-10')
            self.text_hz_125.configure(textvariable=hz125str)
            self.eqslider_4.set(-10)
            hz250str.set('-10')
            self.text_hz_250.configure(textvariable=hz250str)
            self.eqslider_5.set(5)
            hz500str.set('5')
            self.text_hz_500.configure(textvariable=hz500str)
            self.eqslider_6.set(5)
            hz1000str.set('5')
            self.text_hz_1000.configure(textvariable=hz1000str)
            self.eqslider_7.set(10)
            hz2000str.set('10')
            self.text_hz_2000.configure(textvariable=hz2000str)
            self.eqslider_8.set(8)
            hz4000str.set('8')
            self.text_hz_4000.configure(textvariable=hz4000str)
            self.eqslider_9.set(9)
            hz8000str.set('9')
            self.text_hz_8000.configure(textvariable=hz8000str)
            self.eqslider_10.set(-8)
            hz16000str.set('-8')
            self.text_hz_16000.configure(textvariable=hz16000str)
            self.checkbox_1.select()
            self.checkbox_2.deselect()
            self.checkbox_3.deselect()
            self.checkbox_4.deselect()
            self.checkbox_5.deselect()
            self.checkbox_6.deselect()
            self.checkbox_7.deselect()
            self.checkbox_8.deselect()
            self.checkbox_9.deselect()
            self.checkbox_10.deselect()
            self.checkbox_11.deselect()
            self.checkbox_12.deselect()
        if preset == "Fear":
            self.slider.set(0)
            whitenoisestr.set('0')
            self.text_whitenoise.configure(textvariable=whitenoisestr)
            self.slider_2.set(0.94)
            stretchstr.set('0.94')
            self.text_stretch.configure(textvariable=stretchstr)
            self.slider_3.set(1)
            pitchstr.set('1')
            self.text_pitch.configure(textvariable=pitchstr)
            self.slider_4.set(1)
            harmonicstr.set('1')
            self.text_harmonic.configure(textvariable=harmonicstr)
            self.slider_5.set(4)
            percussivestr.set('4')
            self.text_percussive.configure(textvariable=percussivestr)
            self.checkbox_Equalizer.select()
            self.eqslider_1.set(-10)
            hz32str.set('-10')
            self.text_hz_32.configure(textvariable=hz32str)
            self.eqslider_2.set(-10)
            hz63str.set('-10')
            self.text_hz_63.configure(textvariable=hz63str)
            self.eqslider_3.set(15)
            hz125str.set('15')
            self.text_hz_125.configure(textvariable=hz125str)
            self.eqslider_4.set(10)
            hz250str.set('10')
            self.text_hz_250.configure(textvariable=hz250str)
            self.eqslider_5.set(7)
            hz500str.set('7')
            self.text_hz_500.configure(textvariable=hz500str)
            self.eqslider_6.set(5)
            hz1000str.set('5')
            self.text_hz_1000.configure(textvariable=hz1000str)
            self.eqslider_7.set(5)
            hz2000str.set('5')
            self.text_hz_2000.configure(textvariable=hz2000str)
            self.eqslider_8.set(10)
            hz4000str.set('10')
            self.text_hz_4000.configure(textvariable=hz4000str)
            self.eqslider_9.set(-8)
            hz8000str.set('-8')
            self.text_hz_8000.configure(textvariable=hz8000str)
            self.eqslider_10.set(-8)
            hz16000str.set('-8')
            self.text_hz_16000.configure(textvariable=hz16000str)
            self.checkbox_7.select()
            self.checkbox_1.select()
            self.checkbox_2.deselect()
            self.checkbox_3.deselect()
            self.checkbox_4.deselect()
            self.checkbox_5.deselect()
            self.checkbox_6.deselect()
            self.checkbox_8.deselect()
            self.checkbox_9.deselect()
            self.checkbox_10.deselect()
            self.checkbox_11.deselect()
            self.checkbox_12.deselect()
        if preset == "Disgust":
            self.slider.set(0)
            whitenoisestr.set('0')
            self.text_whitenoise.configure(textvariable=whitenoisestr)
            self.slider_2.set(1)
            stretchstr.set('1')
            self.text_stretch.configure(textvariable=stretchstr)
            self.slider_3.set(1.2)
            pitchstr.set('1.2')
            self.text_pitch.configure(textvariable=pitchstr)
            self.slider_4.set(1)
            harmonicstr.set('1')
            self.text_harmonic.configure(textvariable=harmonicstr)
            self.slider_5.set(4)
            percussivestr.set('4')
            self.text_percussive.configure(textvariable=percussivestr)
            self.checkbox_Equalizer.select()
            self.eqslider_1.set(-20)
            hz32str.set('-20')
            self.text_hz_32.configure(textvariable=hz32str)
            self.eqslider_2.set(-10)
            hz63str.set('-10')
            self.text_hz_63.configure(textvariable=hz63str)
            self.eqslider_3.set(-10)
            hz125str.set('-10')
            self.text_hz_125.configure(textvariable=hz125str)
            self.eqslider_4.set(-5)
            hz250str.set('-5')
            self.text_hz_250.configure(textvariable=hz250str)
            self.eqslider_5.set(5)
            hz500str.set('5')
            self.text_hz_500.configure(textvariable=hz500str)
            self.eqslider_6.set(5)
            hz1000str.set('5')
            self.text_hz_1000.configure(textvariable=hz1000str)
            self.eqslider_7.set(10)
            hz2000str.set('10')
            self.text_hz_2000.configure(textvariable=hz2000str)
            self.eqslider_8.set(5)
            hz4000str.set('5')
            self.text_hz_4000.configure(textvariable=hz4000str)
            self.eqslider_9.set(6)
            hz8000str.set('6')
            self.text_hz_8000.configure(textvariable=hz8000str)
            self.eqslider_10.set(-8)
            hz16000str.set('-8')
            self.text_hz_16000.configure(textvariable=hz16000str)
            self.checkbox_7.select()
            self.checkbox_1.select()
            self.checkbox_2.deselect()
            self.checkbox_3.deselect()
            self.checkbox_4.deselect()
            self.checkbox_5.deselect()
            self.checkbox_6.deselect()
            self.checkbox_8.deselect()
            self.checkbox_9.deselect()
            self.checkbox_10.deselect()
            self.checkbox_11.deselect()
            self.checkbox_12.deselect()
        if preset == "Default":
            self.slider.set(0)
            whitenoisestr.set('0')
            self.text_whitenoise.configure(textvariable=whitenoisestr)
            self.slider_2.set(1)
            stretchstr.set('1')
            self.text_stretch.configure(textvariable=stretchstr)
            self.slider_3.set(0)
            pitchstr.set('0')
            self.text_pitch.configure(textvariable=pitchstr)
            self.slider_4.set(1)
            harmonicstr.set('1')
            self.text_harmonic.configure(textvariable=harmonicstr)
            self.slider_5.set(1)
            percussivestr.set('1')
            self.text_percussive.configure(textvariable=percussivestr)
            self.checkbox_Equalizer.deselect()
            self.eqslider_1.set(0)
            hz32str.set('0')
            self.text_hz_32.configure(textvariable=hz32str)
            self.eqslider_2.set(0)
            hz63str.set('0')
            self.text_hz_63.configure(textvariable=hz63str)
            self.eqslider_3.set(0)
            hz125str.set('0')
            self.text_hz_125.configure(textvariable=hz125str)
            self.eqslider_4.set(0)
            hz250str.set('0')
            self.text_hz_250.configure(textvariable=hz250str)
            self.eqslider_5.set(0)
            hz500str.set('0')
            self.text_hz_500.configure(textvariable=hz500str)
            self.eqslider_6.set(0)
            hz1000str.set('0')
            self.text_hz_1000.configure(textvariable=hz1000str)
            self.eqslider_7.set(0)
            hz2000str.set('0')
            self.text_hz_2000.configure(textvariable=hz2000str)
            self.eqslider_8.set(0)
            hz4000str.set('0')
            self.text_hz_4000.configure(textvariable=hz4000str)
            self.eqslider_9.set(0)
            hz8000str.set('0')
            self.text_hz_8000.configure(textvariable=hz8000str)
            self.eqslider_10.set(0)
            hz16000str.set('0')
            self.text_hz_16000.configure(textvariable=hz16000str)
            self.checkbox_1.select()
            self.checkbox_2.deselect()
            self.checkbox_3.deselect()
            self.checkbox_4.deselect()
            self.checkbox_5.deselect()
            self.checkbox_6.deselect()
            self.checkbox_7.deselect()
            self.checkbox_8.deselect()
            self.checkbox_9.deselect()
            self.checkbox_10.deselect()
            self.checkbox_11.deselect()
            self.checkbox_12.deselect()
        if preset=="Robot":
            self.slider.set(0.005)
            whitenoisestr.set('0.005')
            self.text_whitenoise.configure(textvariable=whitenoisestr)
            self.slider_2.set(1)
            stretchstr.set('1')
            self.text_stretch.configure(textvariable=stretchstr)
            self.slider_2.set(0.8)
            pitchstr.set('0.8')
            self.text_pitch.configure(textvariable=pitchstr)
            self.slider_3.set(1.2)
            harmonicstr.set('1.2')
            self.text_harmonic.configure(textvariable=harmonicstr)
            self.slider_4.set(1)
            percussivestr.set('1')
            self.text_percussive.configure(textvariable=percussivestr)
            self.slider_5.set(1)
            self.checkbox_Equalizer.select()
            self.eqslider_1.set(-20)
            hz32str.set('-20')
            self.text_hz_32.configure(textvariable=hz32str)
            self.eqslider_2.set(-20)
            hz63str.set('-20')
            self.text_hz_63.configure(textvariable=hz63str)
            self.eqslider_3.set(-20)
            hz125str.set('-20')
            self.text_hz_125.configure(textvariable=hz125str)
            self.eqslider_4.set(-20)
            hz250str.set('-20')
            self.text_hz_250.configure(textvariable=hz250str)
            self.eqslider_5.set(-20)
            hz500str.set('-20')
            self.text_hz_500.configure(textvariable=hz500str)
            self.eqslider_6.set(20)
            hz1000str.set('20')
            self.text_hz_1000.configure(textvariable=hz1000str)
            self.eqslider_7.set(0)
            hz2000str.set('0')
            self.text_hz_2000.configure(textvariable=hz2000str)
            self.eqslider_8.set(0)
            hz4000str.set('0')
            self.text_hz_4000.configure(textvariable=hz4000str)
            self.eqslider_9.set(0)
            hz8000str.set('0')
            self.text_hz_8000.configure(textvariable=hz8000str)
            self.eqslider_10.set(0)
            hz16000str.set('0')
            self.text_hz_16000.configure(textvariable=hz16000str)
            self.checkbox_1.deselect()
            self.checkbox_2.select()
            self.checkbox_3.deselect()
            self.checkbox_4.deselect()
            self.checkbox_5.deselect()
            self.checkbox_6.deselect()
            self.checkbox_7.deselect()
            self.checkbox_8.deselect()
            self.checkbox_9.deselect()
            self.checkbox_10.deselect()
            self.checkbox_11.deselect()
            self.checkbox_12.deselect()
        if preset=="Kid":
            self.slider.set(0)
            whitenoisestr.set('0')
            self.text_whitenoise.configure(textvariable=whitenoisestr)
            self.slider_2.set(1)
            stretchstr.set('1')
            self.text_stretch.configure(textvariable=stretchstr)
            self.slider_3.set(6)
            pitchstr.set('6')
            self.text_pitch.configure(textvariable=pitchstr)
            self.slider_4.set(2)
            harmonicstr.set('2')
            self.text_harmonic.configure(textvariable=harmonicstr)
            self.slider_5.set(1)
            percussivestr.set('1')
            self.text_percussive.configure(textvariable=percussivestr)
            self.checkbox_Equalizer.select()
            self.eqslider_1.set(0)
            hz32str.set('0')
            self.text_hz_32.configure(textvariable=hz32str)
            self.eqslider_2.set(0)
            hz63str.set('0')
            self.text_hz_63.configure(textvariable=hz63str)
            self.eqslider_3.set(0)
            hz125str.set('0')
            self.text_hz_125.configure(textvariable=hz125str)
            self.eqslider_4.set(0)
            hz250str.set('0')
            self.text_hz_250.configure(textvariable=hz250str)
            self.eqslider_5.set(0)
            hz500str.set('0')
            self.text_hz_500.configure(textvariable=hz500str)
            self.eqslider_6.set(0)
            hz1000str.set('0')
            self.text_hz_1000.configure(textvariable=hz1000str)
            self.eqslider_7.set(0)
            hz2000str.set('0')
            self.text_hz_2000.configure(textvariable=hz2000str)
            self.eqslider_8.set(5)
            hz4000str.set('5')
            self.text_hz_4000.configure(textvariable=hz4000str)
            self.eqslider_9.set(5)
            hz8000str.set('5')
            self.text_hz_8000.configure(textvariable=hz8000str)
            self.eqslider_10.set(5)
            hz16000str.set('5')
            self.text_hz_16000.configure(textvariable=hz16000str)
            self.checkbox_1.deselect()
            self.checkbox_2.deselect()
            self.checkbox_3.deselect()
            self.checkbox_4.deselect()
            self.checkbox_5.deselect()
            self.checkbox_6.deselect()
            self.checkbox_7.deselect()
            self.checkbox_8.deselect()
            self.checkbox_9.deselect()
            self.checkbox_10.deselect()
            self.checkbox_11.deselect()
            self.checkbox_12.deselect()
        if preset=="Ogre":
            self.slider.set(0)
            whitenoisestr.set('0')
            self.text_whitenoise.configure(textvariable=whitenoisestr)
            self.slider_2.set(0.8)
            stretchstr.set('0.8')
            self.text_stretch.configure(textvariable=stretchstr)
            self.slider_3.set(-3)
            pitchstr.set('-3')
            self.text_pitch.configure(textvariable=pitchstr)
            self.slider_4.set(1)
            harmonicstr.set('1')
            self.text_harmonic.configure(textvariable=harmonicstr)
            self.slider_5.set(1)
            percussivestr.set('1')
            self.text_percussive.configure(textvariable=percussivestr)
            self.checkbox_Equalizer.deselect()
            self.eqslider_1.set(0)
            hz32str.set('0')
            self.text_hz_32.configure(textvariable=hz32str)
            self.eqslider_2.set(0)
            hz63str.set('0')
            self.text_hz_63.configure(textvariable=hz63str)
            self.eqslider_3.set(0)
            hz125str.set('0')
            self.text_hz_125.configure(textvariable=hz125str)
            self.eqslider_4.set(0)
            hz250str.set('0')
            self.text_hz_250.configure(textvariable=hz250str)
            self.eqslider_5.set(0)
            hz500str.set('0')
            self.text_hz_500.configure(textvariable=hz500str)
            self.eqslider_6.set(0)
            hz1000str.set('0')
            self.text_hz_1000.configure(textvariable=hz1000str)
            self.eqslider_7.set(0)
            hz2000str.set('0')
            self.text_hz_2000.configure(textvariable=hz2000str)
            self.eqslider_8.set(0)
            hz4000str.set('0')
            self.text_hz_4000.configure(textvariable=hz4000str)
            self.eqslider_9.set(0)
            hz8000str.set('0')
            self.text_hz_8000.configure(textvariable=hz8000str)
            self.eqslider_10.set(0)
            hz16000str.set('0')
            self.text_hz_16000.configure(textvariable=hz16000str)
            self.checkbox_1.select()
            self.checkbox_2.deselect()
            self.checkbox_3.deselect()
            self.checkbox_4.deselect()
            self.checkbox_5.deselect()
            self.checkbox_6.deselect()
            self.checkbox_7.deselect()
            self.checkbox_8.deselect()
            self.checkbox_9.deselect()
            self.checkbox_10.deselect()
            self.checkbox_11.deselect()
            self.checkbox_12.deselect()
        if preset=="Telephone":
            self.slider.set(0)
            whitenoisestr.set('0')
            self.text_whitenoise.configure(textvariable=whitenoisestr)
            self.slider_2.set(1)
            stretchstr.set('1')
            self.text_stretch.configure(textvariable=stretchstr)
            self.slider_3.set(0)
            pitchstr.set('0')
            self.text_pitch.configure(textvariable=pitchstr)
            self.slider_4.set(1)
            harmonicstr.set('1')
            self.text_harmonic.configure(textvariable=harmonicstr)
            self.slider_5.set(1)
            percussivestr.set('1')
            self.text_percussive.configure(textvariable=percussivestr)
            self.checkbox_Equalizer.select()
            self.eqslider_1.set(-20)
            hz32str.set('-20')
            self.text_hz_32.configure(textvariable=hz32str)
            self.eqslider_2.set(-20)
            hz63str.set('-20')
            self.text_hz_63.configure(textvariable=hz63str)
            self.eqslider_3.set(-20)
            hz125str.set('-20')
            self.text_hz_125.configure(textvariable=hz125str)
            self.eqslider_4.set(-20)
            hz250str.set('-20')
            self.text_hz_250.configure(textvariable=hz250str)
            self.eqslider_5.set(-20)
            hz500str.set('-20')
            self.text_hz_500.configure(textvariable=hz500str)
            self.eqslider_6.set(20)
            hz1000str.set('20')
            self.text_hz_1000.configure(textvariable=hz1000str)
            self.eqslider_7.set(20)
            hz2000str.set('20')
            self.text_hz_2000.configure(textvariable=hz2000str)
            self.eqslider_8.set(20)
            hz4000str.set('20')
            self.text_hz_4000.configure(textvariable=hz4000str)
            self.eqslider_9.set(20)
            hz8000str.set('20')
            self.text_hz_8000.configure(textvariable=hz8000str)
            self.eqslider_10.set(-20)
            hz16000str.set('-20')
            self.text_hz_16000.configure(textvariable=hz16000str)
            self.checkbox_1.deselect()
            self.checkbox_2.deselect()
            self.checkbox_3.deselect()
            self.checkbox_4.deselect()
            self.checkbox_5.deselect()
            self.checkbox_6.deselect()
            self.checkbox_7.deselect()
            self.checkbox_8.deselect()
            self.checkbox_9.deselect()
            self.checkbox_10.deselect()
            self.checkbox_11.deselect()
            self.checkbox_12.deselect()
        if preset=="Cave":
            self.slider.set(0)
            whitenoisestr.set('0')
            self.text_whitenoise.configure(textvariable=whitenoisestr)
            self.slider_2.set(1)
            stretchstr.set('1')
            self.text_stretch.configure(textvariable=stretchstr)
            self.slider_3.set(0)
            pitchstr.set('0')
            self.text_pitch.configure(textvariable=pitchstr)
            self.slider_4.set(1)
            harmonicstr.set('1')
            self.text_harmonic.configure(textvariable=harmonicstr)
            self.slider_5.set(1)
            percussivestr.set('1')
            self.text_percussive.configure(textvariable=percussivestr)
            self.checkbox_Equalizer.select()
            self.eqslider_1.set(20)
            hz32str.set('20')
            self.text_hz_32.configure(textvariable=hz32str)
            self.eqslider_2.set(20)
            hz63str.set('20')
            self.text_hz_63.configure(textvariable=hz63str)
            self.eqslider_3.set(-20)
            hz125str.set('-20')
            self.text_hz_125.configure(textvariable=hz125str)
            self.eqslider_4.set(-20)
            hz250str.set('-20')
            self.text_hz_250.configure(textvariable=hz250str)
            self.eqslider_5.set(-20)
            hz500str.set('-20')
            self.text_hz_500.configure(textvariable=hz500str)
            self.eqslider_6.set(-20)
            hz1000str.set('-20')
            self.text_hz_1000.configure(textvariable=hz1000str)
            self.eqslider_7.set(20)
            hz2000str.set('20')
            self.text_hz_2000.configure(textvariable=hz2000str)
            self.eqslider_8.set(-20)
            hz4000str.set('-20')
            self.text_hz_4000.configure(textvariable=hz4000str)
            self.eqslider_9.set(-20)
            hz8000str.set('-20')
            self.text_hz_8000.configure(textvariable=hz8000str)
            self.eqslider_10.set(-20)
            hz16000str.set('-20')
            self.text_hz_16000.configure(textvariable=hz16000str)
            self.checkbox_1.select()
            self.checkbox_2.deselect()
            self.checkbox_3.deselect()
            self.checkbox_4.select()
            self.checkbox_5.deselect()
            self.checkbox_6.deselect()
            self.checkbox_7.deselect()
            self.checkbox_8.deselect()
            self.checkbox_9.deselect()
            self.checkbox_10.deselect()
            self.checkbox_11.deselect()
            self.checkbox_12.deselect()

    def rec(self):
        audio = pyaudio.PyAudio()
        stream = audio.open(format=pyaudio.paInt16, channels=1, rate=44100,
                            input=True, frames_per_buffer=1024)
        
        frames = []
        
        start = time.time()        
        
        while self.recording:
            data = stream.read(1024)
            frames.append(data)
            
            passed = time.time() - start
            secs = passed % 60
            mins = passed // 60 
            hours = mins // 60
                    
        stream.stop_stream()
        stream.close()
        audio.terminate()
        
        exists = True
        i = 1
        while exists:
            if os.path.exists(self.path + f"/rec{i}.wav"):
                i+= 1
            else:
                exists = False
                sound_file = wave.open(self.path + f"/rec{i}.wav", "wb")
                sound_file.setnchannels(1)
                sound_file.setsampwidth(audio.get_sample_size(pyaudio.paInt16))
                sound_file.setframerate(44100)
                sound_file.writeframes(b"".join(frames))
                sound_file.close()
    
    def choosePath(self):
        self.path = filedialog.askdirectory()
    

if __name__ == "__main__":
    app = App()
    app.mainloop()
